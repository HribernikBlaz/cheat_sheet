...
namespace naloga05_NadgradnjaXMLSheme
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                Console.WriteLine("11) Validiraj XML dokumente");
                ...
                switch (izbira)
                {
                    ...
                    case "11": ValidirajVseDokumente(); break;
                    case "0":
        ...
        // VALIDACIJA
        // ---------------------------------
        static void ValidirajVseDokumente()
        {
            Console.WriteLine("\n========== VALIDACIJA XML DOKUMENTOV ==========");
            Console.WriteLine("\nValidiram artikle...");
            ValidirajXML(potDoArtiklov);

            Console.WriteLine("\nValidiram dobavitelje...");
            ValidirajXML(potDoDobaviteljev);

            if (validacijaOK)
            {
                Console.WriteLine("\nOba dokumenta sta veljavna glede na XSD shemo!");
            }
            else
            {
                Console.WriteLine("\nValidacija ni uspela. Prosim, preverite napake zgoraj.");
            }
        ...
        private static void ValidationCallBack(object sender, ValidationEventArgs e)
        {
            ...
            // napredna shema
            string napaka = $"{e.Severity}: {e.Message}";
            validacijskeNapake.Add(napaka);
            Console.WriteLine($"  ❌ {napaka}");
            validacijaOK = false;
        }
...
<dobavitelji xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance""
        ...
        static void IzpisArtiklov()
        {
            ...
            foreach (XmlNode artikel in artikli)
            {
                ...
                string razpolozljiv = artikel.SelectSingleNode("razpolozljiv").Attributes["stanje"]?.Value ?? "?";
        ...
        static void DodajArtikel()
        {
            ...
            if (!System.Text.RegularExpressions.Regex.IsMatch(id, @"^A[0-9]{3,}$"))
            {
                Console.WriteLine("\nNeveljaven format ID! Mora biti A + vsaj 3 številke (npr. A001)");
                Console.ReadKey();
                Console.Clear();
                return;
            }
            ...
            try
            {
                ...
                UstvariArtikel(doc, root, id, kategorija, naziv, cena.ToString(CultureInfo.InvariantCulture),
                   zaloga.ToString(), dobaviteljId, datum, razpolozljiv);
